...
import { Budget, fixedText, fixedWrap, list, listItem, listItems } from '../SideBar.css';
...
const FixedExpenses = () => {
  ...
  //고정 지출
  const { data: fixedExpense, isLoading: isFixedExpense, error: fixedExpenseError } = useQuery<FixedExpense, Error>({
  ...
  if (!fixedExpense || fixedExpense.fixed_expenses_per_list.length === 0) return <div className={Budget} >등록된 고정지출이 없슈</div>;
      ...
      <ul className={list}>
        {fixedExpense.fixed_expenses_per_list.map((item, index) => (
          <li className={listItems} key={index}>
            <Text as='p' className={listItem}>{categoryMap[item.category] || '기타'}</Text>
